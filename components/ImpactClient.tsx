...
export default function ImpactClient() {
  ...
  const inputStyle = { padding: "16px 20px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 3, color: "var(--ink)", fontSize: 15, fontFamily: "var(--font-body)", width: "100%", transition: "border-color 0.2s", outline: "none" };
  ...
  return (
    <div style={{ paddingTop: 72 }}>
      ...
      <section style={{ padding: "100px 32px 80px", borderBottom: "1px solid var(--border)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, width: 600, height: 500, background: "radial-gradient(ellipse at left bottom, rgba(16,185,129,0.06) 0%, transparent 70%)" }} />
        <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative" }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(44px, 6vw, 84px)", fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 0.95, marginBottom: 32, maxWidth: 760 }}>
            Africa Has<br />the Talent.<br /><span style={{ color: "var(--emerald)" }}>We Build</span><br /><span style={{ color: "var(--accent)" }}>the Systems.</span>
          </h1>
          <p style={{ fontSize: 18, color: "var(--ink-muted)", maxWidth: 520, lineHeight: 1.8 }}>
      ...
      {/* Story */}
      <section ref={s1} style={{ padding: "100px 32px", background: "var(--bg-soft)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }} className="hero-grid">
            <div>
              ...
              <h2 className="fade-up" style={{ fontFamily: "var(--font-display)", fontSize: "clamp(28px, 3vw, 44px)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1, marginBottom: 28 }}>Founded in Rulindo.<br /><span style={{ color: "var(--accent)" }}>Built for a continent.</span></h2>
              <p className="fade-up" style={{ fontSize: 15, color: "var(--ink-muted)", lineHeight: 1.9, marginBottom: 24 }}>
              ...
              <p className="fade-up" style={{ fontSize: 15, color: "var(--ink-muted)", lineHeight: 1.9, marginBottom: 40 }}>
              ...
              <div className="fade-up" style={{ display: "flex", gap: 32 }}>
                <div>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 32, fontWeight: 800, color: "var(--accent)" }}>2025</div>
                  <div style={{ fontSize: 12, color: "var(--ink-faint)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em" }}>Founded</div>
                </div>
                <div>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 32, fontWeight: 800, color: "var(--accent)" }}>Rulindo</div>
                  <div style={{ fontSize: 12, color: "var(--ink-faint)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em" }}>Headquarters</div>
                </div>
                <div>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 32, fontWeight: 800, color: "var(--accent)" }}>Africa</div>
                  <div style={{ fontSize: 12, color: "var(--ink-faint)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em" }}>Market</div>
                </div>
              </div>
            </div>
            {/* Milestones */}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                {[
                  ...
                  <div key={m.label} className="fade-up card" style={{ padding: 32, textAlign: "center" }}>
                    <div style={{ fontSize: 24, marginBottom: 12, color: "var(--accent)" }}>{m.icon}</div>
                    <div style={{ fontFamily: "var(--font-display)", fontSize: 36, fontWeight: 800, color: "var(--accent)", marginBottom: 8 }}>{m.number}</div>
                    <div style={{ fontSize: 12, color: "var(--ink-muted)", fontWeight: 600 }}>{m.label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 2 }} className="two-grid">
            {[
              ...
              <div key={v.title} className="fade-up card" style={{ padding: "44px 40px" }}>
                <div style={{ fontSize: 28, marginBottom: 24, color: "var(--accent)" }}>{v.icon}</div>
                <h3 style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 700, letterSpacing: "-0.02em", marginBottom: 14 }}>{v.title}</h3>
                <p style={{ fontSize: 14, color: "var(--ink-muted)", lineHeight: 1.85 }}>{v.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Contact */}
      <section id="contact" ref={s3} style={{ padding: "clamp(70px, 10vw, 110px) clamp(16px, 5vw, 32px)", background: "var(--bg-page)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }} className="hero-grid">
          <div>
            <div className="fade-up tag" style={{ marginBottom: 20 }}>Enterprise Contact</div>
            <h2 className="fade-up" style={{ fontFamily: "var(--font-display)", fontSize: "clamp(32px, 4vw, 56px)", fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1.0, marginBottom: 24 }}>
              Plan your next<br />software initiative<br /><span className="gradient-text">with confidence.</span>
            </h2>
            <p className="fade-up" style={{ fontSize: 15, color: "var(--ink-muted)", lineHeight: 1.8, marginBottom: 44 }}>
              Tell us your business goals, technical constraints, and timeline. We will respond with a practical plan covering scope, architecture direction, and delivery approach.
            </p>
            <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {[
                { icon: "◎", label: "General Inquiries", value: "contact@sybellasystems.co.rw" },
                { icon: "◈", label: "Location", value: "Kigali, Rwanda" },
                { icon: "⬡", label: "Response SLA", value: "Initial response within 24-48 hours" },
              ].map(c => (
                <div key={c.label} style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                  <div style={{ width: 36, height: 36, background: "var(--accent-soft)", border: "1px solid var(--border-accent)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: "var(--accent)", flexShrink: 0 }}>{c.icon}</div>
                  <div>
                    <div style={{ fontSize: 11, color: "var(--ink-faint)", fontFamily: "var(--font-display)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>{c.label}</div>
                    <div style={{ fontSize: 14, color: "var(--ink-muted)" }}>{c.value}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="fade-up">
            {sent ? (
              <div style={{ padding: 60, background: "var(--accent-soft)", border: "1px solid var(--border-accent)", borderRadius: 12, textAlign: "center" }}>
                <div style={{ fontSize: 40, marginBottom: 16 }}>◎</div>
                <div style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 700, color: "var(--accent)", marginBottom: 8 }}>Message received.</div>
                <div style={{ fontSize: 14, color: "var(--ink-muted)", marginBottom: 24 }}>We'll respond within 24–48 hours.</div>
                <button onClick={() => setSent(false)} style={{ padding: "10px 20px", background: "var(--accent)", color: "var(--ink-on-accent)", border: "none", borderRadius: 9999, cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Send another message</button>
              </div>
            ) : (
              <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 16, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "clamp(18px, 3vw, 28px)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <textarea name="message" required placeholder="Describe what you're building and how we can help..." rows={6} value={form.message} onChange={handle} style={{ ...inputStyle, resize: "vertical" }}
                  onFocus={e => e.target.style.borderColor = "var(--accent)"}
                  onBlur={e => e.target.style.borderColor = "var(--border)"} />
                {error && <div style={{ fontSize: 13, color: "var(--red)", padding: "12px 16px", background: "var(--red-dim)", borderRadius: 2 }}>{error}</div>}
                <button type="submit" className="btn-primary" style={{ justifyContent: "center", padding: "18px 32px", fontSize: 14 }} disabled={loading}>
                  {loading ? "Sending..." : "Send Inquiry →"}
                </button>
                <p style={{ fontSize: 12, color: "var(--ink-faint)", textAlign: "center" }}>contact@sybellasystems.co.rw · Initial response in 24-48 hours</p>
              </form>
            )}
          </div>
        </div>
